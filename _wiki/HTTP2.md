...
updated : 2018-01-31 11:40:49 +0900
tags    : http/2
toc     : true
public  : true
parent  : Concept
...
### 헤더 압축이 필요한 이유
Mozilla의 Patrick McManus는 평균 페이지로드 헤더의 영향을 계산하여 이를 분명하게 보여주었습니다.

<---- 여기까지 ---> 
페이지에 80 개의 assets가 있다고 가정하면 (현재의 Web은 겸손입니다) 각 요청에는 1400 바이트의 헤더가 있습니다 (Cookie, Referer 등 덕분에 희귀하지 않습니다). 헤더를 "와이어에"에서 내기위한 왕복이 있습니다. 이것은 응답 시간을 계산하지 않습니다. 클라이언트에서 검색하면됩니다.

이것은 TCP의 Slow Start 메커니즘이 원인입니다. 그것은 어떤 패킷이 승인되었는지에 따라 새로운 연결에 패킷을 전송하기 때문에 처음 몇 번 왕복으로 보낼 수있는 패킷 수를 효과적으로 제한합니다.

이와 비교하여 헤더를 가볍게 압축하고도 1 회 왕복 (아마 1 패킷)에서 와이어에 도달 할 수 있습니다.

이러한 오버 헤드 모바일 클라이언트에 미치는 영향을 고려하는 경우에 그렇습니다. 모바일 클라이언트는 일반적으로 양호한 상태로 수백 밀리 초 왕복 대기 시간이 있습니다.

### 왜 HPACK입니까?
SPDY / 2는 헤더 압축을 위해 각 방향으로 하나의 GZIP 컨텍스트를 사용하여 제안되었습니다. 이것은 구현이 간단하고 효율적인이었습니다.

이후 암호화 내부에서 스트림 압축 (GZIP 등)을 사용하는 것에 대해 주요 공격이 문서화되어 있습니다. 범죄.

CRIME는 암호화 된 스트림에 데이터를 주입하여 평문을 "프로브"하고 복구하는 능력을 가진 공격자가있을 가능성이 있습니다. 이것은 웹이므로, JavaScript는 이것을 가능하게하고 TLS로 보호 된 HTTP 자원에 CRIME를 사용하여 쿠키와 인증 토큰 복구 시연되었습니다.

그 결과, GZIP 압축을 사용할 수 없습니다. 이 유스 케이스에 적합하고 안전하게 사용할 수있는 다른 알고리즘이 발견되지 않기 때문에 거친 곡물에서 동작하는 새로운 헤더 고유의 압축 방식을 만들었습니다. HTTP 헤더는 메시지간에 변경되지 않는 경우가 많기 때문에 압축 효율이 합리적이고 더 안전합니다.

### HTTP/2에서 쿠키 (또는 다른 헤더)를 향상시킬 수 있습니까?
이 노력은 HTTP 프로토콜의 수정, 즉 HTTP 헤더 및 메소드 등이 HTTP의 의미를 변경하는 것이 아니라, 어떻게 "와이어에"두는지를 확인하기 위해 수립되었습니다.

HTTP는 널리 사용되고 있기 때문입니다. 이 버전의 HTTP를 사용하여 새 상태 메커니즘 (설명되어있는 하나의 예)를 도입하고, 핵심 메소드를 변경하거나해서 (아마도 이것은 아직 제안되지 않습니다) 새로운 프로토콜이 기존 의 Web과 호환되지 않습니다.

특히 HTTP/1 HTTP/2로 변환이 가능하고, 정보를 잃지 않고 돌아갈 수 있도록하고 싶다고 생각하고 있습니다. 헤더를 "정리"시작하면 (대부분의 경우 HTTP 헤더가 꽤 혼란임을 동의합니다) 기존 Web의 많은 상호 운용성 문제가 있습니다.

이렇게하는 것은 새로운 프로토콜 채택에 반대하는 것만 마찰이 생깁니다.

그것은 모든 HTTP 작업 부회가 HTTP/2뿐만 아니라 모든 HTTP를 담당하고 있다는 것입니다. 따라서 기존의 Web과 호환성이있는 한 버전에 의존하지 않는 새로운 메커니즘으로 작업 할 수 있습니다.

### HTTP 브라우저 이외의 사용자는 어떻습니까?
비 HTTP 응용 프로그램에서도 이미 HTTP를 사용하는 경우는 HTTP/2도 사용할 수있을 것입니다.

조기의 의견은 HTTP/2가 HTTP "API"에 대해 뛰어난 성능 특성을 가지고있는 것입니다. 왜냐하면 API는 설계에서 요구 오버 헤드 등을 고려할 필요가 없기 때문입니다.

하지만 우리가 고려하고 개선의 주요 초점은 프로토콜의 핵심 예제이기 때문에 일반적인 브라우징의 사용 예이다.

우리의 헌장은 이에 대해 이렇게 말합니다 :

결과 규격은 공통의 목적을 달성 할 것으로 기대되고 있습니다
기존의 HTTP 배포. 특히 Web 브라우징 (데스크톱과
모바일), 비 브라우저 ( "HTTP API"), Web 서비스 (다양한 스케일) 및
중개 (프록시 기업 방화벽 "반대"프록시 및 내용
배달 네트워크). 마찬가지로, 현재와 미래의 의미 론적
HTTP/1.x (헤더 방법 상태 코드 캐시 지시문 등)
새로운 프로토콜에서 지원됩니다.

여기에는 지정되지 않은 행동이 HTTP의 사용은 포함되지 않는 점에 유의하십시오
(예를 들어, 제한 또는 클라이언트 선호도 등의 연결 상태,
및 "차단"프록시). 이러한 사용은 궁극적으로 사용 가능하게되는 경우가되지 않는 경우가 있습니다
제품.

### HTTP/2는 암호화가 필요합니까?
워킹 그룹은 광범위한 논의 후 새로운 프로토콜로 암호화 (예 : TLS)을 사용하도록 합의했다.

그러나 암호화 된 연결에 사용되는 경우는 HTTP/2 만 지원하고 현재는 암호화되지 않은 HTTP/2를 지원하는 브라우저가 아닙니다.
